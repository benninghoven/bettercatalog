...
#PREREQ AND COREQUISIITES
SELECT PREREQDEPT, PREREQNUM, PREREQCOURSELETTER, ISCOREQ FROM COURSEPREREQ
WHERE COURSEPREREQ.COURSEDEPT = @DEPTCODE AND COURSEPREREQ.COURSENUM = @COURSENUMBER AND COURSEPREREQ.COURSELETTER = @COURSELETTER
UNION
SELECT COURSEDEPT, COURSENUM, COURSELETTER, ISCOREQ FROM COURSEPREREQ
WHERE PREREQDEPT = @DEPTCODE AND PREREQNUM = @COURSENUMBER AND PREREQCOURSELETTER = @COURSELETTER AND ISCOREQ = TRUE;
...
#PARAMATERIZED VERSION: "SELECT DEPTCODE, COURSENUM, COURSELETTER, COURSENAME FROM COURSE WHERE DEPTCODE LIKE CONCAT(UPPER(%s), '%') AND COURSENUM LIKE CONCAT(%s, '__%') AND COURSELETTER LIKE CONCAT(UPPER(%s), '%');"
